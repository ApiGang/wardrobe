...
SAVED_OUTPUT_FOLDER = "Created_Outfits"
...
class WardrobeApp:
    ...
    def generate_outfit(self,top_image_path,bottom_image_path):
        try:
            # top_index= top_index.index(top_image_path)
            # bottom_index= bottom_index.index(top_image_path)

            # top_image_path = self.top_images[top_index]
            # bottom_image_path = self.bottom_images[bottom_index]
            ...
            print("top_image_path ",top_image_path)
            print("bottom_image_path ",bottom_image_path)

             # Fetch top and bottom styles and colors from the database
            User_top_style = self.db_handler.get_top_genre_by_path(top_image_path)
            User_top_color = self.db_handler.get_top_color_by_path(top_image_path)
            User_bottom_style = self.db_handler.get_bottom_genre_by_path(bottom_image_path)
            ...
            face_image_path = r'C:\Users\Pratham Jain\SisterDear\WardrobeApp\faces\Kriti.jpg'    

            #  # Get the absolute paths for the images
            # top_image_path = os.path.abspath(top_image_path)
            ...
            # with open(top_image_path, "rb") as top_image_file:
            #     top_image_data = base64.b64encode(top_image_file.read()).decode("utf-8")
            # with open(bottom_image_path, "rb") as bottom_image_file:
            #     bottom_image_data = base64.b64encode(bottom_image_file.read()).decode("utf-8")
            ...
            with open(bottom_image_path, "rb") as bottom_image_file:
                bottom_image_data = base64.b64encode(bottom_image_file.read()).decode("utf-8")
            with open(face_image_path, "rb") as face_image_path:
                face_image_path = base64.b64encode(face_image_path.read()).decode("utf-8")

            dataURI_top = f"data:image/{top_image_path.split('.')[-1]};base64,{top_image_data}"
            dataURI_bottom = f"data:image/{bottom_image_path.split('.')[-1]};base64,{bottom_image_data}"
            dataURI_face = f"data:image/{face_image_path.split('.')[-1]};base64,{face_image_path}"
            ...
            img= Image.open(output_image[0])
            # Assuming output_image is the generated image
            plt.imshow(img)
            plt.axis('off')  # Turn off axis labels
            plt.show()

            saved_output_path = os.path.join(os.getcwd(), SAVED_OUTPUT_FOLDER)
            os.makedirs(saved_output_path, exist_ok=True)

            outfit_name = filedialog.asksaveasfilename(
            initialdir=saved_output_path,
            title="Save Your Generated Outfit",
            filetypes=[("Image files", "*.png;*.jpg;*.jpeg;*.gif")],
            defaultextension=".png")

            if outfit_name:
                copy2(img, os.path.join(saved_output_path, f"{outfit_name}_.png"))
    ...
    def create_outfit(self):
        self.generate_outfit(self.tops_image_path,self.bottom_image_path)
    ...
    def store_clothing_details(self, name, clothing_type, size, color, brand, genre, fabric,path):
        try:
            # # Replace 'column_name' with actual column names in your database schema
            # query = "INSERT INTO clothing_table (id, name, type, size, color, brand, genre, fabric, path) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
            # values = (name, clothing_type, size, color, brand, genre, fabric, path)
            # self.db_handler.store_clothing_details(query, values)
